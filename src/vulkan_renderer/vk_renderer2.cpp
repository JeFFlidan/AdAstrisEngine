...
namespace ad_astris
{
	...
	void VkRenderer::draw_deferred_pass(VkCommandBuffer cmd)
	{
		...
		ShaderPass* prevMaterial = nullptr;
		...
		if (material != nullptr)
		{
			auto shaderPass = material->original->passShaders[MeshpassType::Deferred];
			vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, shaderPass->pipeline);
	...
	void VkRenderer::draw_forward_pass(VkCommandBuffer cmd, uint32_t swapchainImageIndex)
	{
	...
	void VkRenderer::draw_tranparency_pass(VkCommandBuffer cmd)
	{
		...
		ShaderPass* shaderPass = _transparencyData.geometryPass;
		vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, shaderPass->pipeline);
	...
	void VkRenderer::draw_compositing_pass(VkCommandBuffer cmd)
	{
		...
		if (material != nullptr)
		{
			auto meshPass = material->original->passShaders[MeshpassType::Forward];
	...
	void VkRenderer::draw_taa_pass(VkCommandBuffer cmd)
	{
		vkCmdPipelineBarrier(
			cmd,
		...
		if (material != nullptr)
		{
			auto shaderPass = material->original->passShaders[MeshpassType::Forward];
			vkCmdBindPipeline(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, shaderPass->pipeline);
			vkCmdBindDescriptorSets(cmd, VK_PIPELINE_BIND_POINT_GRAPHICS, shaderPass->layout, 0, 1, &globalSet, 0, nullptr);
		}
		else
	...
	void VkRenderer::draw_final_quad(VkCommandBuffer cmd, uint32_t swapchainImageIndex)
	{
		VkImageMemoryBarrier imgBarrier1 = vkinit::image_barrier(
			_temporalFilter.taaOldColorAttach.imageData.image,
			VK_ACCESS_TRANSFER_WRITE_BIT,
	...
	void VkRenderer::submit(VkCommandBuffer cmd, uint32_t swapchainImageIndex)
	{
		VkSubmitInfo submit{};
	...
	void VkRenderer::bake_shadow_maps(VkCommandBuffer cmd)
	{
		...
		if (std::find(dirMaps.begin(), dirMaps.end(), true) != dirMaps.end())
		{
			...
			draw_shadow_pass(cmd, MeshpassType::DirectionalShadow);
		...
		if (std::find(pointMaps.begin(), pointMaps.end(), true) != pointMaps.end())
		{
			...
			draw_shadow_pass(cmd, MeshpassType::PointShadow);
		...
		if (std::find(spotMaps.begin(), spotMaps.end(), true) != spotMaps.end())
		{
			...
			draw_shadow_pass(cmd, MeshpassType::SpotShadow);

			spotMaps = { false };
		}
	}

	void VkRenderer::draw_shadow_pass(VkCommandBuffer cmd, MeshpassType passType)
	{
	...
	void VkRenderer::draw_objects_in_shadow_pass(VkCommandBuffer cmd, VkDescriptorSet globalDescriptorSet, RenderScene::MeshPass& meshPass, uint32_t id)
	{
	...
	void VkRenderer::reallocate_light_buffer(ActorType lightType)
	{
		RenderScene* scene = &_renderScene;
	...
	void VkRenderer::culling(RenderScene::MeshPass& meshPass, VkCommandBuffer cmd, CullParams cullParams)
	{
		VkDescriptorBufferInfo objectDataBufferInfo = _renderScene._objectDataBuffer.get_info(true);
	...
	void VkRenderer::prepare_data_for_drawing(VkCommandBuffer cmd)
	{
		RenderScene* scene = &_renderScene;
	...
	void VkRenderer::prepare_per_frame_data(VkCommandBuffer cmd)
	{
	...
	void VkRenderer::depth_reduce(VkCommandBuffer cmd)
	{	
	...
	void VkRenderer::prepare_gpu_indirect_buffer(VkCommandBuffer cmd, RenderScene::MeshPass& meshPass)
	{
