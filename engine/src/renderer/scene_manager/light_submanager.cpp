...
void LightSubmanager::subscribe_to_events()
{
	...
	events::EventDelegate<ecore::PointLightCreatedEvent> pointLightCreatedDelegate = [&](ecore::PointLightCreatedEvent& event)
	{
		...
		pointLight.locationAndAttenuationRadius = XMFLOAT4(
			context->transformComponent.location.x,
			context->transformComponent.location.y,
			context->transformComponent.location.z,
			context->attenuationRadiusComponent.attenuationRadius);
	...
	events::EventDelegate<ecore::SpotLightCreatedEvent> spotLigthCreatedDelegate = [&](ecore::SpotLightCreatedEvent& event)
	{
		...
		spotLight.locationAndDistance = XMFLOAT4(
			context->transformComponent.location.x,
			context->transformComponent.location.y,
			context->transformComponent.location.z,
			context->attenuationRadiusComponent.attenuationRadius);
		...
		spotLight.rotationAndInnerConeRadius = XMFLOAT4(rx, ry, rz, innerConeRadius);
